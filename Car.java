...
class Question1{
    public static void main(String[] args) {
        ...
        List<String> ownersWithNoCars = CarRepository.getOwners().stream().filter(owner -> owner.getCars() == null || owner.getCars().isEmpty()).map(Owner::getName)
                .collect(Collectors.toList());

        System.out.println("1. Owners with no cars:");
        System.out.println(ownersWithNoCars);

        Map<String, Integer> ownerCarCount = CarRepository.getOwners().stream().collect(Collectors.toMap(
                        Owner::getName,
        ...
        Set<Car> ownedCars = CarRepository.getOwners().stream().filter(owner -> owner.getCars() != null).flatMap(owner -> owner.getCars().stream())
                .collect(Collectors.toSet());
        List<Car> notOwnedCars = CarRepository.getCars().stream().filter(car -> !ownedCars.contains(car)).toList();
